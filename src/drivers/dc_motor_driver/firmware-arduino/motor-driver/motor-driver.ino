...
void loop() {
  ...
  {
  digitalWrite(CHA_LPWM, values[0] > 0);
  digitalWrite(CHA_RPWM, !(values[0] > 0));
  digitalWrite(CHB_LPWM, values[1] > 0);
  digitalWrite(CHB_RPWM, !(values[1] > 0));
  analogWrite(CHA_PWM, abs(values[0])*2);
  analogWrite(CHB_PWM, abs(values[1])*2);
  }
  else
  {
