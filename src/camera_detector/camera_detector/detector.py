    ...
    def _update_costmap(self):
        ...
        for detection in detections:
            if detection['cls'] == 'person': #ONLY FOR DEBUG WITH yolo8n.pt TODO eliminare
                ...
                # Calcola gli angoli in radianti    
                ...
                # try:
                #     self.get_logger().info(f"Filtered ranges: {len(filtered_ranges)}, {min(filtered_ranges)}, {max(filtered_ranges)}")
                #     self.get_logger().info(f"Filtered angles: {len(angles[filt_angles])}, {min(angles[filt_angles])}, {max(angles[filt_angles])}")
                # except:
                #     self.get_logger().info(f"Filtered ranges: {len(filtered_ranges)}")
                #     self.get_logger().info(f"Filtered angles: {len(angles[filt_angles])}")

                # if left_bound_lidar_rad > 0 and right_bound_lidar_rad > 0:
                      #angoli < left_bound_lidar_rad
                #     #angoli > right_bound_lidar_rad
                # if left_bound_lidar_rad < 0 and right_bound_lidar_rad < 0:
                #     #angoli < left_bound_lidar_rad
                #     #angoli > right_bound_lidar_rad
                ...
                # for angle, range in zip(self.latest_scan_data['angles'], self.latest_scan_data['ranges']):
                    
                #     if angle >= left_bound_lidar_rad or angle <= right_bound_lidar_rad:
                #         continue   
                ...
                #     # print(f"LiDAR coordinates: ({x_lidar}, {y_lidar}), Grid coordinates: ({grid_x}, {grid_y})")
                #     # print(f"Angle: {angle}, Range: {range}")
